...
public class AccountTransfer {
    ...
    private static List<Account> createAccounts(int accountNum){
        ...
        for(int i=0;i<accountNum;i++){
            int asset = new Random().nextInt(100);
        ...
        public void run() {
            ...
            if(getLockFlag){
                ...
                try{
                    ...
                    if(dealAmount>0){
                        log.debug("[start]处理{}向{}转账，交易金额为{},当前{}余额{},{}余额{}",out.name,come.name,dealAmount,out.name,out.remains,come.name,come.remains);
                        isOutValid = out.remains.get()- (long) dealAmount >=0;
                        if(isOutValid){//出账后不是负资产
                            out.toOut(dealAmount);
                            come.toCome(dealAmount);
                            log.debug("[end]处理{}向{}转账，交易金额为{},当前{}余额{},{}余额{}",out.name,come.name,dealAmount,out.name,out.remains,come.name,come.remains);
                        }else{//出账后是负资产
                            log.debug("[end]===不足===无法处理{}向{}转账，当前{}余额{},转账金额{}",out.name,come.name,out.name,out.remains,dealAmount);
                        }
...
/*                        log.debug("[start]处理{}向{}转账，交易金额为{},当前{}余额{},{}余额{}",out.name,come.name,dealAmount,out.name,out.remains,come.name,come.remains);
                        ...
                        log.debug("[start]处理{}向{}转账，交易金额为{},当前{}余额{},{}余额{}",come.name,out.name,-dealAmount,come.name,come.remains,out.name,out.remains);
                        isOutValid = come.remains.get()+ (long) dealAmount >=0;
                        if(isOutValid) {//出账后不是负资产
                            out.toOut(dealAmount);
                            come.toCome(dealAmount);
                            log.debug("[end]处理{}向{}转账，交易金额为{},当前{}余额{},{}余额{}",come.name,out.name,-dealAmount,come.name,come.remains,out.name,out.remains);
                        }else {
                            log.debug("[end]===不足===无法处理{}向{}转账，当前{}余额{},转账金额{}",come.name,out.name,come.name,come.remains,-dealAmount);
                        }
