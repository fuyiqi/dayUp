package EngineSkill.ProCon.Instance;
    ...
    public void put(Egg egg){
        ...
        try{
            if(occupiedCapacity<initialCapacity){
                lineQueue.offer(egg);
                occupiedCapacity++;
                System.out.println("一颗"+egg.toString()+"【装入】篮子，当前已占用槽位量："+occupiedCapacity);
                informChildren.signalAll();
            }else {//篮子已满
                try {
                    informHens.await();
                    System.out.println("篮子已满，母鸡："+Thread.currentThread().getId()+"进入休息");
                }catch (Exception e2){
    ...
    public void fetch(){
        ...
        try {
            if(occupiedCapacity>0){
                ...
                occupiedCapacity--;
                System.out.println("一颗"+egg.toString()+"【移出】篮子，当前篮子已占用槽位数量："+occupiedCapacity);
                informHens.signalAll();
            }else {//篮子已空
                try{
                    informChildren.await();
                    System.out.println("篮子已空，小孩："+Thread.currentThread().getId()+"进入休息");
                }catch (Exception e2){
